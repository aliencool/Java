...
public class MyTank extends Tank 
{
	...
	//连发子弹
	Vector<Bullet> vb=new Vector<Bullet>();
	Vector<Bullet> vb1=new Vector<Bullet>();
	//子弹的类型
	int type=2;
	...
	public void shooter()
	{
		switch(this.direct)
		{
			case 0://向上
				...
				b1=new Bullet(x+10, y,0,36);
				vb.add(b);
				vb1.add(b1);
				break;
			case 1:
				b=new Bullet(x+30, y+10,1,36);
				b1=new Bullet(x+30, y+10,1,36);
				vb.add(b);
				vb1.add(b1);
				break;
			case 2://向下
				b=new Bullet(x+10, y+30,2,36);
				b1=new Bullet(x+10, y+30,2,36);
				vb.add(b);
				vb1.add(b1);
				break;
			case 3://向左
				b=new Bullet(x, y+10,3,36);
				b1=new Bullet(x, y+10,3,36);
				vb.add(b);
				vb1.add(b1);
				break;
			}
		Thread t=new Thread(b);//每个子弹都是新的线程
		t.start();//但按下J的时候就初始化射出位置，然后，启动线程不断运动
		Thread t1=new Thread(b1);//每个子弹都是新的线程
		t1.start();//但按下J的时候就初始化射出位置，然后，启动线程不断运动
	}
...
class Bullet implements Runnable
{
	...
	public Bullet(int x, int y, boolean isLive) {
		super();
		this.x = x;
		this.y = y;
		this.isLive = isLive;
...
class Bullet_type implements Runnable 
{
	//子弹类型飘来时候的坐标
	int x;	
	int y;
	int type=2;
	boolean islive=true;
	public Bullet_type(int x, int y, boolean islive) {
